    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1">
                Selamat datang kembali, <span class="font-semibold text-[#405078]">{{ Auth::user()->username }}</span>
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div class="bg-white border border-[#D1D9EB] rounded-2xl shadow-sm p-6 lg:sticky lg:top-8">
            ...
            <form action="{{ route('piket.dispen.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label for="nis" class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Pilih Siswa</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="jam_keluar_rencana" class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Jam Keluar</label>
                        <input type="time" name="jam_keluar_rencana" id="jam_keluar_rencana" required
                            class="block w-full px-3.5 py-2.5 bg-[#F8FAFC] border border-[#D1D9EB] rounded-xl text-sm text-[#1E2538] focus:outline-none focus:border-[#405078] focus:ring-2 focus:ring-[#405078]/15 transition-all cursor-pointer">
                    </div>
                    <div>
                        <label for="jam_kembali_rencana" class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Jam Kembali (Opsional)</label>
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm text-gray-600">
                            @forelse ($dispenHariIni as $index => $d)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="py-4 px-6 font-semibold text-[#1E2538]">{{ $d->nis }}</td>
