...
<div class="space-y-4 max-w-5xl mx-auto">
    ...
    <form action="{{ route('guru.jurnal.store') }}" method="POST" class="space-y-4">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden">
            <div class="px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                <div>
                    <h3 class="font-bold text-slate-900 dark:text-white text-xs flex items-center space-x-1.5 uppercase tracking-wider">
                        <i data-lucide="users-check" class="w-3.5 h-3.5 text-slate-700 dark:text-slate-300"></i>
                        <span>Presensi Siswa di Jam Mapel Ini</span>
                    </h3>
                </div>
                <div class="flex items-center space-x-2">
                    <button type="button" onclick="setAllAttendance('Hadir')" class="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 rounded-lg text-[11px] font-bold hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-all flex items-center space-x-1 cursor-pointer" title="Tandai semua siswa hadir">
            ...
            @else
                ...
                <!-- MOBILE ATTENDANCE CARDS (KHUSUS MOBILE ONLY) -->
                <div class="block md:hidden divide-y divide-slate-100 dark:divide-slate-800">
                    @foreach ($siswaDiKelas as $idx => $s)
                        <div class="p-3 space-y-2 bg-white dark:bg-[#1E2538]">
                            <div class="flex items-start justify-between">
                                <div class="flex items-center space-x-2">
                                    <span class="w-6 h-6 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold text-[11px] flex items-center justify-center shrink-0 tabular-nums">
                                        {{ $idx + 1 }}
                                    </span>
                                    <div>
                                ...
                                @if ($s->info_status)
                                    <span class="inline-flex items-center px-2 py-0.5 bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 text-[10px] font-bold rounded space-x-1">
                                        <i data-lucide="info" class="w-3 h-3"></i>
                                        <span>{{ $s->info_status }}</span>
                                    </span>
                                @endif
                            ...
                            <!-- Touch-Friendly Attendance Pills for Mobile -->
                            <div class="grid grid-cols-5 gap-1 pt-0.5">
                                <label class="cursor-pointer select-none">
                                    <input type="radio" name="ketidakhadiran[{{ $s->nis }}]" value="Hadir" {{ $s->auto_status === 'Hadir' ? 'checked' : '' }} class="peer sr-only">
                                    <span class="min-h-[34px] w-full rounded-lg text-[11px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 peer-checked:bg-emerald-600 peer-checked:border-emerald-600 peer-checked:text-white transition-all flex items-center justify-center shadow-2xs">
                                        Hadir
                                    </span>
                                </label>
                                <label class="cursor-pointer select-none">
                                    <input type="radio" name="ketidakhadiran[{{ $s->nis }}]" value="Sakit" {{ $s->auto_status === 'Sakit' ? 'checked' : '' }} class="peer sr-only">
                                    <span class="min-h-[34px] w-full rounded-lg text-[11px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 peer-checked:bg-blue-600 peer-checked:border-blue-600 peer-checked:text-white transition-all flex items-center justify-center shadow-2xs">
                                        Sakit
                                    </span>
                                </label>
                                <label class="cursor-pointer select-none">
                                    <input type="radio" name="ketidakhadiran[{{ $s->nis }}]" value="Izin" {{ $s->auto_status === 'Izin' ? 'checked' : '' }} class="peer sr-only">
                                    <span class="min-h-[34px] w-full rounded-lg text-[11px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 peer-checked:bg-amber-500 peer-checked:border-amber-500 peer-checked:text-white transition-all flex items-center justify-center shadow-2xs">
                                        Izin
                                    </span>
                                </label>
                                <label class="cursor-pointer select-none">
                                    <input type="radio" name="ketidakhadiran[{{ $s->nis }}]" value="Dispen" {{ $s->auto_status === 'Dispen' ? 'checked' : '' }} class="peer sr-only">
                                    <span class="min-h-[34px] w-full rounded-lg text-[11px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 peer-checked:bg-indigo-600 peer-checked:border-indigo-600 peer-checked:text-white transition-all flex items-center justify-center shadow-2xs">
                                        Dispen
                                    </span>
                                </label>
                                <label class="cursor-pointer select-none">
                                    <input type="radio" name="ketidakhadiran[{{ $s->nis }}]" value="Alpa" class="peer sr-only">
                                    <span class="min-h-[34px] w-full rounded-lg text-[11px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 peer-checked:bg-rose-600 peer-checked:border-rose-600 peer-checked:text-white transition-all flex items-center justify-center shadow-2xs">
                                        Alpa
                                    </span>
                                </label>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('input[type="radio"][name^="ketidakhadiran["]').forEach(radio => {
            radio.addEventListener('change', function() {
                if (this.checked) {
                    const selector = `input[type="radio"][name="${this.name}"][value="${this.value}"]`;
