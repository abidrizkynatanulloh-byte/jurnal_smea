...
<div class="space-y-4">
    ...
    <!-- Alert / Notifikasi -->
    @if(session('success'))
        <div class="p-2.5 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xs font-semibold flex items-center space-x-2 shadow-2xs">
            <i data-lucide="check-circle" class="w-3.5 h-3.5 text-emerald-600"></i>
            <span>{{ session('success') }}</span>
        </div>
    @endif
    @if(session('error'))
        <div class="p-2.5 bg-rose-50 border border-rose-200 text-rose-800 rounded-lg text-xs font-semibold flex items-center space-x-2 shadow-2xs">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
        ...
        <!-- FORM TAMBAH -->
        <div class="bg-white border border-slate-200 rounded-xl shadow-xs p-3.5 lg:sticky lg:top-18 space-y-2.5">
            <div class="flex items-center space-x-2 pb-2 border-b border-slate-100">
                <div class="w-5 h-5 rounded bg-[#1E2538] text-white flex items-center justify-center">
                    <i data-lucide="user-plus" class="w-3 h-3"></i>
                </div>
                <h3 class="font-bold text-slate-900 text-xs uppercase tracking-wider">Angkat Menjadi Waka</h3>
            </div>
            
            <form action="{{ route('admin.waka.store') }}" method="POST" class="space-y-2.5">
                @csrf
                ...
                <div>
                    ...
                    <select name="tipe_waka" required class="block w-full h-8 px-2.5 bg-white border border-slate-200 rounded-lg text-xs text-slate-800 focus:outline-none focus:border-[#1E2538] cursor-pointer">
        ...
        <div class="lg:col-span-2 space-y-3.5">
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden">
                <div class="px-3.5 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                    ...
                    <span class="text-[11px] text-slate-500 font-semibold">{{ count($wakaSiswa) }} Penugasan</span>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse text-xs">
                        <thead>
                            <tr class="bg-white text-slate-500 text-[11px] font-bold uppercase tracking-wider border-b border-slate-200">
                                <th class="py-2 px-3.5">Nama Waka</th>
                                <th class="py-2 px-3.5 text-center w-28">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                            @forelse($wakaSiswa as $ws)
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    <td class="py-2 px-3.5 font-medium text-slate-900">{{ $ws->guru->nama_guru ?? 'Data Guru Tidak Ditemukan' }}</td>
                                    <td class="py-2 px-3.5 text-center">
                                        <form action="{{ route('admin.waka.destroy', $ws->id) }}" method="POST" onsubmit="return confirm('Berhentikan wewenang Waka dan kembalikan menjadi Guru biasa?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="h-6.5 px-2.5 border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-md text-xs font-semibold transition-colors cursor-pointer">Berhentikan</button>
                                        </form>
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="2" class="py-5 text-center text-slate-400 italic text-xs">Belum ada Waka Kesiswaan</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden">
                <div class="px-3.5 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                    ...
                    <span class="text-[11px] text-slate-500 font-semibold">{{ count($wakaGuru) }} Penugasan</span>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse text-xs">
                        <thead>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                            @forelse($wakaGuru as $wg)
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    <td class="py-2 px-3.5 font-medium text-slate-900">{{ $wg->guru->nama_guru ?? 'Data Guru Tidak Ditemukan' }}</td>
                                    <td class="py-2 px-3.5 text-center">
                            ...
                            @empty
                                <tr><td colspan="2" class="py-5 text-center text-slate-400 italic text-xs">Belum ada Waka Kurikulum</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
