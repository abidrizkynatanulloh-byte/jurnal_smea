    ...
    <div class="bg-white border border-[#19140015] rounded-2xl shadow-sm p-5">
        <form action="{{ route('admin.rekap.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
            ...
            <div class="flex space-x-2 w-full sm:w-auto">
                ...
                <a href="{{ route('admin.rekap.index') }}" class="px-5 py-2.5 border border-gray-250 bg-white hover:bg-gray-50 text-gray-600 rounded-xl text-sm font-semibold transition-colors flex items-center justify-center space-x-1">
                ...
                <tbody class="divide-y divide-[#19140010] text-sm text-gray-600">
                    @forelse ($jurnalTersimpan as $jt)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="py-4 px-6 font-medium text-dark max-w-sm truncate" title="{{ $jt->materi }}">{{ $jt->materi ?? '-' }}</td>
                            <td class="py-4 px-6 text-gray-500 max-w-xs truncate" title="{{ $jt->catatan }}">{{ $jt->catatan ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
