...
<div class="flex-1 flex flex-col min-h-0 space-y-3">
    ...
    <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch" id="masterDataGrid">
        ...
        <div id="formPanel" class="bg-white border border-slate-200/90 rounded-xl p-4.5 shadow-2xs space-y-4 overflow-y-auto h-full max-h-full">
            ...
            <form action="{{ route('admin.jadwal.store') }}" method="POST" class="space-y-3.5">
                ...
                <div>
                    <label for="id_kelas" class="block text-xs font-semibold text-slate-700 mb-1.5">Kelas *</label>
                ...
                <div class="grid grid-cols-2 gap-2.5">
                    <div>
                        <label for="hari" class="block text-xs font-semibold text-slate-700 mb-1.5">Hari *</label>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label for="jam_mulai" class="block text-xs font-semibold text-slate-700 mb-1.5">Mulai</label>
                            <input type="number" name="jam_mulai" id="jam_mulai" min="1" max="15" value="{{ old('jam_mulai', 1) }}" required
                                class="w-full h-10 px-2.5 bg-white border border-slate-200 rounded-lg text-xs text-slate-900 text-center font-mono focus:outline-none focus:border-slate-800">
                        </div>
                        <div>
                            <label for="jam_selesai" class="block text-xs font-semibold text-slate-700 mb-1.5">Selesai</label>
                ...
                <div>
                    <label for="kode_mapel" class="block text-xs font-semibold text-slate-700 mb-1.5">Mata Pelajaran *</label>
                ...
                <div>
                    <label for="id_guru" class="block text-xs font-semibold text-slate-700 mb-1.5">Guru Pengampu *</label>
                ...
                <div>
                    <label for="id_ruangan" class="block text-xs font-semibold text-slate-700 mb-1.5">Ruangan Kelas / Lab</label>
        ...
        <!-- BAGIAN 2: DATA TABLE CARD (PROPORSI & GAYA SESUAI GAMBAR 1) -->
        <div id="tableCol" class="lg:col-span-2 flex flex-col h-full min-h-0">
            <div class="bg-white border border-slate-200/90 rounded-xl shadow-2xs overflow-hidden flex flex-col h-full min-h-0">
                <!-- Top Control Bar (Sesuai Gambar 1 - Search + Semua Hari + Semua Kelas) -->
                <div class="shrink-0 px-4 py-3 border-b border-slate-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-2.5">
                    <form action="{{ route('admin.jadwal.index') }}" method="GET" class="flex flex-1 flex-col sm:flex-row items-center gap-2.5 w-full">
                        ...
                        <div class="w-full sm:w-36">
                            <select name="hari" onchange="this.form.submit()" class="w-full h-10 px-3 bg-white border border-slate-200 rounded-lg text-xs text-slate-800 font-medium focus:outline-none focus:border-slate-800 cursor-pointer">
                        ...
                        @if (request('search') || request('hari') || request('id_kelas'))
                            <a href="{{ route('admin.jadwal.index') }}" class="h-10 px-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-semibold flex items-center transition-colors shrink-0">
                                Reset
                            </a>
                        @endif
                    </form>
                ...
                <div class="flex-1 overflow-y-auto overflow-x-auto min-h-0">
                    <table class="w-full text-left border-collapse text-xs" id="tableJadwal">
                        <thead class="sticky top-0 bg-white border-b border-slate-200 z-10 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                            <tr>
                                <th class="py-3 px-4 w-32 bg-white">WAKTU</th>
                                <th class="py-3 px-4 w-32 bg-white">KELAS & RUANG</th>
                                <th class="py-3 px-4 bg-white">MATA PELAJARAN</th>
                                <th class="py-3 px-4 bg-white">GURU PENGAMPU</th>
                                <th class="py-3 px-4 text-center w-16 bg-white">AKSI</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 text-slate-700" id="jadwalTbody">
                            @forelse ($jadwalList as $j)
                                <tr class="hover:bg-slate-50/80 transition-colors jadwal-row" 
                                    ...
                                    <td class="py-3 px-4 font-medium text-slate-800 text-xs leading-tight">
                                    ...
                                    <td class="py-3 px-4 text-center">
                                        <form action="{{ route('admin.jadwal.destroy', $j->id_jadwal) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jadwal ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-7 h-7 rounded-lg border border-slate-200 hover:border-rose-200 hover:bg-rose-50 text-slate-400 hover:text-rose-600 flex items-center justify-center transition-colors shadow-2xs cursor-pointer" title="Hapus Jadwal">
