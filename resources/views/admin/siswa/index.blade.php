...
<div class="flex-1 flex flex-col min-h-0 space-y-3">
    ...
    <div class="shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div class="flex items-center space-x-3">
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Data Siswa & Rombel</h1>
        ...
        <div class="flex items-center space-x-2">
            ...
            <a href="{{ route('admin.siswa.trash') }}" class="h-9 px-3.5 border border-rose-200 bg-rose-50/80 hover:bg-rose-100 text-rose-700 rounded-lg text-xs font-semibold transition-colors flex items-center space-x-1.5 shadow-2xs">
    ...
    <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch" id="masterDataGrid">
        ...
        <!-- BAGIAN 1: FORM TAMBAH SISWA (UKURAN & PROPORSI SESUAI GAMBAR 1) -->
        <div id="formCol" class="bg-white border border-slate-200/90 rounded-xl p-4.5 shadow-2xs space-y-3.5 overflow-y-auto h-full max-h-full">
            ...
            <form action="{{ route('admin.siswa.store') }}" method="POST" class="space-y-3">
                ...
                <div>
                    ...
                    <input type="text" name="nis" value="{{ old('nis') }}" placeholder="Contoh: 23456" required
                        class="w-full h-10 px-3 bg-white border border-slate-200 rounded-lg text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-800 font-mono transition-colors">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Nomor Induk Siswa Nasional (NISN) *</label>
                ...
                <div class="grid grid-cols-2 gap-2.5">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Rombel / Kelas *</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Nomor WhatsApp Wali Siswa</label>
                    <input type="text" name="no_hp_wali" value="{{ old('no_hp_wali') }}" placeholder="08xxxxxxxxxx (Untuk Notifikasi WA)"
                        class="w-full h-10 px-3 bg-white border border-slate-200 rounded-lg text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-800 font-mono transition-colors">
                </div>
        ...
        <div id="tableCol" class="lg:col-span-2 flex flex-col h-full min-h-0">
            <div class="bg-white border border-slate-200/90 rounded-xl shadow-2xs overflow-hidden flex flex-col h-full min-h-0">
                ...
                <div class="shrink-0 px-4 py-3 border-b border-slate-200 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
                    <form action="{{ route('admin.siswa.index') }}" method="GET" class="flex flex-1 flex-col sm:flex-row items-center gap-2.5 w-full">
                        ...
                        <div class="w-full sm:w-40">
                            <select name="id_kelas" onchange="this.form.submit()" class="w-full h-10 px-3 bg-white border border-slate-200 rounded-lg text-xs text-slate-800 font-medium focus:outline-none focus:border-slate-800 cursor-pointer">
                        ...
                        <div class="w-full sm:w-36">
                            <select name="jenis_kelamin" onchange="this.form.submit()" class="w-full h-10 px-3 bg-white border border-slate-200 rounded-lg text-xs text-slate-800 font-medium focus:outline-none focus:border-slate-800 cursor-pointer">
                        ...
                        @if (request('search') || request('id_kelas') || request('jenis_kelamin'))
                            <a href="{{ route('admin.siswa.index') }}" class="h-10 px-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-semibold flex items-center transition-colors shrink-0">
                                Reset
                            </a>
                        @endif
                    </form>
                ...
                <div class="flex-1 overflow-y-auto overflow-x-auto min-h-0">
                    <table class="w-full text-left border-collapse text-xs" id="tableSiswa">
                        <thead class="sticky top-0 bg-white border-b border-slate-200 z-10 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                            <tr>
                                <th class="py-3 px-3 text-center w-12 bg-white">No</th>
                                <th class="py-3 px-3 w-24 bg-white">NIS</th>
                                <th class="py-3 px-3 w-28 bg-white">NISN</th>
                                <th class="py-3 px-3 bg-white">NAMA LENGKAP SISWA</th>
                                <th class="py-3 px-2 text-center w-14 bg-white">L/P</th>
                                <th class="py-3 px-3 w-28 bg-white">KELAS</th>
                                <th class="py-3 px-3 w-32 bg-white">NO. HP WALI</th>
                                <th class="py-3 px-3 text-center w-20 bg-white">AKSI</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 text-slate-700" id="siswaTbody">
                            @forelse ($siswaList as $idx => $s)
                                <tr class="hover:bg-slate-50/80 transition-colors siswa-row" data-search="{{ strtolower($s->nama_siswa . ' ' . $s->nis . ' ' . $s->nisn . ' ' . ($s->kelas ? $s->kelas->nama_kelas : '')) }}">
                                    ...
                                    <td class="py-2.5 px-3 font-semibold text-slate-900 text-xs font-mono tabular-nums">{{ $s->nis }}</td>
                                    <td class="py-2.5 px-3 text-xs text-slate-500 font-mono tabular-nums">{{ $s->nisn }}</td>
                                    <td class="py-2.5 px-3 font-medium text-slate-900 text-xs leading-tight">{{ $s->nama_siswa }}</td>
                                    <td class="py-2.5 px-2 text-center">
                                        @if($s->jenis_kelamin === 'P')
                                            <span class="inline-flex items-center justify-center w-6 h-6 rounded-md text-xs font-bold bg-pink-50 text-pink-700 border border-pink-200" title="Perempuan">P</span>
                                        @else
                                            <span class="inline-flex items-center justify-center w-6 h-6 rounded-md text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200" title="Laki-laki">L</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="py-2.5 px-3 text-xs text-slate-500 font-mono tabular-nums">
                                        {{ $s->no_hp_wali ?: '-' }}
                                    </td>
                                    <td class="py-2.5 px-3 text-center">
                                        <div class="flex items-center justify-center space-x-1.5">
                                            <button type="button" 
                                                ...
                                                class="w-7 h-7 rounded-lg border border-slate-200 hover:border-slate-300 hover:bg-slate-100 text-slate-600 flex items-center justify-center transition-colors shadow-2xs cursor-pointer" title="Edit Siswa">
                                            ...
                                            <form action="{{ route('admin.siswa.destroy', $s->nis) }}" method="POST" onsubmit="return confirm('Pindahkan siswa {{ addslashes($s->nama_siswa) }} ke sampah?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="w-7 h-7 rounded-lg border border-slate-200 hover:border-rose-200 hover:bg-rose-50 text-slate-400 hover:text-rose-600 flex items-center justify-center transition-colors shadow-2xs cursor-pointer" title="Hapus Siswa">
        ...
        <form id="formEditSiswa" method="POST" class="space-y-3">
            ...
            <div>
                ...
                <input type="text" name="nis" id="edit_nis" required readonly
                    class="w-full h-10 px-3 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-600 font-mono focus:outline-none cursor-not-allowed">
            </div>
            ...
            <div class="flex items-center justify-end space-x-2 pt-2.5 border-t border-slate-100">
                <button type="button" onclick="closeEditModal()" class="h-9 px-4 border border-slate-200 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 cursor-pointer">
                    Batal
                </button>
