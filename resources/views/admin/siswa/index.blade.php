        ...
        <div class="bg-white border border-[#D1D9EB] rounded-2xl shadow-sm p-6 lg:sticky lg:top-8">
            ...
            <form action="{{ route('admin.siswa.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nis" id="nis" value="{{ old('nis') }}" placeholder="Contoh: 202401001" required 
                ...
                <div>
                    <label for="jenis_kelamin" class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Jenis Kelamin</label>
                ...
                <div class="pt-2 border-t border-gray-100">
                    <p class="text-[10px] font-bold text-[#8697C3] uppercase tracking-wider mb-2">Informasi Akun Wali Murid</p>
                    <div>
                        <label for="no_hp_wali" class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">No HP / WA Wali Murid</label>
                        <input type="text" name="no_hp_wali" id="no_hp_wali" value="{{ old('no_hp_wali') }}" placeholder="08xxxxxxxxxx (Untuk akun ortu)"
                            class="block w-full px-3.5 py-2.5 bg-[#F8FAFC] border border-[#D1D9EB] rounded-xl text-sm text-[#1E2538] placeholder-gray-400 focus:outline-none focus:border-[#405078] focus:ring-2 focus:ring-[#405078]/15 transition-all">
                    </div>
                </div>
                ...
                <form action="{{ route('admin.siswa.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
                    ...
                    <div class="flex space-x-2 w-full sm:w-auto">
                        ...
                        @if (request('search') || request('id_kelas'))
                            <a href="{{ route('admin.siswa.index') }}" class="px-4 py-2 border border-[#D1D9EB] hover:bg-gray-50 text-gray-500 hover:text-[#1E2538] rounded-xl text-sm font-semibold transition-colors flex items-center justify-center">
                                Reset
                            </a>
                        @endif
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-[#F8FAFC] text-gray-500 text-xs font-semibold uppercase tracking-wider border-b border-[#D1D9EB]">
                                ...
                                <th class="py-4 px-6">No HP Wali</th>
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm text-gray-600">
                            @forelse ($siswaList as $s)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="py-4 px-6 font-semibold text-[#1E2538]">{{ $s->nis }}</td>
                                    <td class="py-4 px-6 font-medium text-[#1E2538]">{{ $s->nama_siswa }}</td>
                                    <td class="py-4 px-6">
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        <span class="px-2 py-0.5 {{ $s->jenis_kelamin == 'L' ? 'bg-blue-50 text-blue-700' : 'bg-pink-50 text-pink-700' }} rounded text-xs font-bold">
                                            {{ $s->jenis_kelamin }}
                                        </span>
                                    </td>
                ...
                @if ($siswaList->hasPages())
                    <div class="px-6 py-4 bg-[#F8FAFC] border-t border-[#D1D9EB] flex items-center justify-between">
                        <div class="text-xs text-gray-500">
                            Menampilkan halaman <span class="font-semibold text-[#1E2538]">{{ $siswaList->currentPage() }}</span> dari <span class="font-semibold text-[#1E2538]">{{ $siswaList->lastPage() }}</span>
                        </div>
                        <div class="inline-flex space-x-1.5">
                            @if ($siswaList->previousPageUrl())
                                <a href="{{ $siswaList->previousPageUrl() }}" class="px-3.5 py-1.5 border border-[#D1D9EB] bg-white hover:bg-gray-50 text-gray-600 rounded-lg text-xs font-semibold transition-colors flex items-center space-x-1">
                                    <i data-lucide="chevron-left" class="w-3.5 h-3.5"></i>
                                    <span>Sebelumnya</span>
                                </a>
                            @endif
                            @if ($siswaList->nextPageUrl())
                                <a href="{{ $siswaList->nextPageUrl() }}" class="px-3.5 py-1.5 border border-[#D1D9EB] bg-white hover:bg-gray-50 text-gray-600 rounded-lg text-xs font-semibold transition-colors flex items-center space-x-1">
