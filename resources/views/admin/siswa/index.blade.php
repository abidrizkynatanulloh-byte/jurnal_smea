...
<div class="space-y-5">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0">
        <div>
            <h1 class="text-xl font-extrabold text-[#1E2538] tracking-tight">Master Data Siswa</h1>
            <p class="text-xs text-slate-500 mt-0.5">Kelola data seluruh peserta didik, kelas, dan akun akses wali murid</p>
        </div>
        <div class="flex items-center space-x-2.5">
            ...
            <a href="{{ route('admin.siswa.trash') }}" class="px-3 py-1.5 border border-rose-200 bg-rose-50/60 hover:bg-rose-100 text-rose-700 rounded-xl text-xs font-semibold transition-colors flex items-center space-x-1.5 shadow-2xs">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start" id="masterDataGrid">
        ...
        <div id="formCol" class="lg:col-span-1 bg-white border border-slate-200 rounded-2xl p-4.5 shadow-xs space-y-3 lg:sticky lg:top-20 z-10 transition-all duration-300">
            <div class="flex items-center justify-between pb-2.5 border-b border-slate-100">
                <div class="flex items-center space-x-2">
                    <i data-lucide="user-plus" class="w-4 h-4 text-[#405078]"></i>
                    <h3 class="font-bold text-[#1E2538] text-xs uppercase tracking-wider">Tambah Siswa Baru</h3>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-3.5 shadow-xs sticky top-20 z-20">
                <form action="{{ route('admin.siswa.index') }}" method="GET" class="flex flex-col sm:flex-row gap-2.5 items-end">
                    ...
                    @if (request('search') || request('id_kelas'))
                        <a href="{{ route('admin.siswa.index') }}" class="px-3 py-1.5 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-xl text-xs font-semibold flex items-center justify-center">
                            Reset
                        </a>
                    @endif
                </form>
                    ...
                    <table class="w-full text-left border-collapse text-xs" id="tableSiswa">
                        <thead class="sticky top-0 bg-[#F8FAFC] z-10">
                            <tr class="text-slate-500 font-semibold uppercase tracking-wider border-b border-slate-200">
                                ...
                                <th class="py-2.5 px-2.5 text-center w-12">L/P</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-600" id="siswaTbody">
                            @forelse ($siswaList as $idx => $s)
                                <tr class="hover:bg-slate-50/60 transition-colors siswa-row" data-search="{{ strtolower($s->nama_siswa . ' ' . $s->nis . ' ' . $s->nisn . ' ' . ($s->kelas ? $s->kelas->nama_kelas : '')) }}">
                                    <td class="py-2 px-3 text-center font-medium text-slate-400 text-xs">
                                        {{ $siswaList->firstItem() + $idx }}
                                    </td>
                                    <td class="py-2 px-3 font-bold text-[#405078] text-xs">{{ $s->nis }}</td>
                                    <td class="py-2 px-3 text-xs font-semibold text-slate-500">{{ $s->nisn }}</td>
                                    <td class="py-2 px-3 font-bold text-[#1E2538] text-xs leading-tight">{{ $s->nama_siswa }}</td>
                                    <td class="py-2 px-2.5 text-center">
                                        <span class="px-1.5 py-0.5 rounded text-[10.5px] font-extrabold {{ $s->jenis_kelamin === 'P' ? 'bg-pink-50 text-pink-700' : 'bg-blue-50 text-blue-700' }}">
                                            {{ $s->jenis_kelamin ?? 'L' }}
                                        </span>
                                    </td>
                                    <td class="py-2 px-3">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold bg-[#405078]/10 text-[#405078]">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="py-8 text-center text-slate-400 italic text-xs">
...
<div id="modalEditSiswa" class="fixed inset-0 z-50 bg-black/40 backdrop-blur-2xs flex items-center justify-center hidden p-4">
            ...
            <div class="grid grid-cols-2 gap-2.5">
                <div>
                    ...
                    <select name="jenis_kelamin" id="edit_jk" required
                        class="block w-full px-2.5 py-1.5 bg-[#F8FAFC] border border-slate-200 rounded-xl text-xs text-[#1E2538] focus:outline-none focus:border-[#405078]">
