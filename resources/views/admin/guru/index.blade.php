...
<div class="flex-1 flex flex-col min-h-0 space-y-3">
    ...
    <div class="shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <div class="flex items-center space-x-2">
            ...
            <a href="{{ route('admin.guru.trash') }}" class="h-9 px-3.5 border border-rose-200 bg-rose-50/80 hover:bg-rose-100 text-rose-700 rounded-lg text-xs font-semibold transition-colors flex items-center space-x-1.5 shadow-2xs">
    ...
    <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch" id="masterDataGrid">
        ...
        <!-- BAGIAN 1: FORM TAMBAH GURU (UKURAN & PROPORSI SESUAI GAMBAR 1) -->
        ...
        <div id="tableCol" class="lg:col-span-2 flex flex-col h-full min-h-0">
            <div class="bg-white border border-slate-200/90 rounded-xl shadow-2xs overflow-hidden flex flex-col h-full min-h-0">
                ...
                <div class="shrink-0 px-4 py-3 border-b border-slate-200 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
                    <form action="{{ route('admin.guru.index') }}" method="GET" class="flex flex-1 flex-col sm:flex-row items-center gap-2.5 w-full">
                        ...
                        <div class="w-full sm:w-44">
                            <select name="jabatan" onchange="this.form.submit()" class="w-full h-10 px-3 bg-white border border-slate-200 rounded-lg text-xs text-slate-800 font-medium focus:outline-none focus:border-slate-800 cursor-pointer">
                        ...
                        @if (request('search') || request('jabatan'))
                            <a href="{{ route('admin.guru.index') }}" class="h-10 px-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-semibold flex items-center transition-colors shrink-0">
                                Reset
                            </a>
                        @endif
                    </form>
                ...
                <div class="flex-1 overflow-y-auto overflow-x-auto min-h-0">
                    <table class="w-full text-left border-collapse text-xs" id="tableGuru">
                        <thead class="sticky top-0 bg-white border-b border-slate-200 z-10 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                            <tr>
                                <th class="py-3 px-4 text-center w-12 bg-white">No</th>
                                <th class="py-3 px-4 bg-white">NAMA LENGKAP & NIP</th>
                                <th class="py-3 px-4 w-44 bg-white">JABATAN / ROLE</th>
                                <th class="py-3 px-4 w-36 bg-white">NO. HP (WHATSAPP)</th>
                                <th class="py-3 px-4 text-center w-20 bg-white">AKSI</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 text-slate-700" id="guruTbody">
                            @forelse ($guruList as $idx => $g)
                                <tr class="hover:bg-slate-50/80 transition-colors guru-row" data-search="{{ strtolower($g->nama_guru . ' ' . $g->nip . ' ' . $g->jabatan) }}">
                                    ...
                                    <td class="py-3 px-4 text-xs text-slate-600 font-mono tabular-nums">
                                        {{ $g->no_hp ?: '-' }}
                                    </td>
                                    <td class="py-3 px-4 text-center">
                                        <div class="flex items-center justify-center space-x-1.5">
                                            <button type="button" 
                                                ...
                                                class="w-7 h-7 rounded-lg border border-slate-200 hover:border-slate-300 hover:bg-slate-100 text-slate-600 flex items-center justify-center transition-colors shadow-2xs cursor-pointer" title="Edit Guru">
                                            ...
                                            <form action="{{ route('admin.guru.destroy', $g->id_guru) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data {{ addslashes($g->nama_guru) }}?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="w-7 h-7 rounded-lg border border-slate-200 hover:border-rose-200 hover:bg-rose-50 text-slate-400 hover:text-rose-600 flex items-center justify-center transition-colors shadow-2xs cursor-pointer" title="Hapus Guru">
    ...
    <div class="bg-white border border-slate-200 rounded-xl shadow-xl max-w-md w-full p-5 space-y-3.5">
        ...
        <form id="formEditGuru" method="POST" class="space-y-3">
            ...
            <div class="flex items-center justify-end space-x-2 pt-2.5 border-t border-slate-100">
                <button type="button" onclick="closeEditGuruModal()" class="h-9 px-4 border border-slate-200 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 cursor-pointer">
                    Batal
                </button>
