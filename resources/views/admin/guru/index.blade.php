...
<div class="flex-1 flex flex-col min-h-0 space-y-2.5">
    ...
    <div class="shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <div class="flex items-center space-x-2 shrink-0 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            ...
            <a href="{{ route('admin.guru.trash') }}" class="h-10 px-4 border border-rose-200 bg-rose-50/90 hover:bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 dark:border-rose-900 rounded-xl text-xs font-bold transition-all flex items-center space-x-2 shadow-2xs whitespace-nowrap shrink-0">
    ...
    <div class="flex-1 min-h-0 flex flex-col" id="masterDataGrid">
        ...
        <div id="tableCol" class="flex-1 flex flex-col h-full min-h-0">
            <div class="bg-white border border-slate-200/90 rounded-xl shadow-2xs overflow-hidden flex flex-col h-full min-h-0">
                ...
                <div class="shrink-0 px-3.5 py-1.5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#1E2538] flex items-center justify-between gap-2 overflow-x-auto no-scrollbar">
                    <form action="{{ route('admin.guru.index') }}" method="GET" class="flex flex-1 flex-row items-center gap-2 w-full min-w-0">
                        <div class="relative flex-1 min-w-[120px]">
                            ...
                            <input type="text" name="search" id="liveSearchGuru" value="{{ request('search') }}" placeholder="Cari Nama, NIP, atau Jabatan..." 
                                class="w-full h-8 pl-8 pr-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-slate-800 transition-colors">
                        </div>

                        <div class="w-36 sm:w-40 shrink-0">
                            <select name="jabatan" onchange="this.form.submit()" class="w-full h-8 px-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-800 dark:text-slate-200 font-medium focus:outline-none focus:border-slate-800 cursor-pointer">
                        ...
                        @if (request('search') || request('jabatan'))
                            <a href="{{ route('admin.guru.index') }}" class="h-8 px-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-semibold flex items-center transition-colors shrink-0">
                                Reset
                            </a>
                        @endif
                    </form>
...
<!-- Modal Tambah Guru / Pegawai Baru -->
<div id="modalTambahGuru" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 overflow-y-auto">
    <div class="bg-white dark:bg-[#1C2433] rounded-2xl max-w-lg w-full p-5 sm:p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 my-auto">
        ...
        <form action="{{ route('admin.guru.store') }}" method="POST" class="space-y-3.5">
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Nomor HP (WhatsApp)</label>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Password Akun Login *</label>
                <div class="relative">
                    <input type="password" name="password" id="password_tambah_guru" placeholder="Minimal 4 karakter" required
                        class="w-full h-10 pl-3 pr-9 bg-white dark:bg-[#1A2230] border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-blue-600 transition-colors">
                    <button type="button" onclick="togglePasswordVisibility('password_tambah_guru', 'eye_tambah_guru')" 
                        class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-700 cursor-pointer" title="Lihat/Sembunyikan Password">
                        <i data-lucide="eye" id="eye_tambah_guru" class="w-4 h-4"></i>
                    </button>
                </div>
            </div>

            <div class="flex items-center justify-end space-x-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                <button type="button" onclick="closeTambahGuruModal()" class="h-10 px-4 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 transition-colors cursor-pointer">
                    Batal
                </button>
...
<script>
    function openTambahGuruModal() {
        document.getElementById('modalTambahGuru').classList.remove('hidden');
    }

    function closeTambahGuruModal() {
        document.getElementById('modalTambahGuru').classList.add('hidden');
    }
