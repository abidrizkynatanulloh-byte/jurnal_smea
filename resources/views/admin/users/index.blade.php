...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-dark tracking-tight">Kelola Pengguna (Users)</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div class="bg-white border border-[#19140015] rounded-2xl shadow-sm p-6 lg:sticky lg:top-8">
            ...
            <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label for="role" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Pilih Peran (Role)</label>
                ...
                <div>
                    <label for="nama" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nama Lengkap</label>
                ...
                <div>
                    <label for="no_hp" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nomor HP (Opsional)</label>
            ...
            <div class="bg-white border border-[#19140015] rounded-2xl shadow-sm p-5">
                <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
                    ...
                    <div class="flex space-x-2 w-full sm:w-auto">
                        ...
                        @if (request('search'))
                            <a href="{{ route('admin.users.index') }}" class="px-4 py-2 border border-gray-200 hover:bg-gray-50 text-gray-500 hover:text-dark rounded-xl text-sm font-semibold transition-colors flex items-center justify-center">
                                Reset
                            </a>
                        @endif
                    </div>
                        ...
                        <tbody class="divide-y divide-[#19140010] text-sm text-gray-600">
                            @forelse ($users as $index => $u)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="py-4 px-6 font-semibold text-dark">{{ $u->username }}</td>
                                    ...
                                    <td class="py-4 px-6">
                                        <span class="px-2.5 py-1 bg-gray-100 rounded-md text-xs font-bold text-gray-700 uppercase">
                                            {{ str_replace('_', ' ', $u->role) }}
                                        </span>
                                    </td>
                                    <td class="py-4 px-6">
                                        @if ($u->is_active)
                                            <span class="inline-flex items-center px-2.5 py-0.5 bg-green-50 text-green-700 text-xs font-bold rounded-full space-x-1">
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        <div class="flex items-center justify-center space-x-2.5">
                                            <a href="{{ route('admin.users.edit', $u->id) }}" class="p-1.5 text-blue-500 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer" title="Edit Pengguna">
                                            ...
                                            <form action="{{ route('admin.users.destroy', $u->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menonaktifkan akun {{ $u->username }}?');" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-1.5 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors cursor-pointer" title="Hapus / Nonaktifkan Pengguna">
                ...
                @if ($users->hasPages())
                    <div class="px-6 py-4 bg-gray-50 border-t border-[#19140015] flex items-center justify-between">
                        <div class="text-xs text-gray-500">
                            Menampilkan halaman <span class="font-semibold text-dark">{{ $users->currentPage() }}</span> dari <span class="font-semibold text-dark">{{ $users->lastPage() }}</span>
                        </div>
                        <div class="inline-flex space-x-1.5">
                            @if ($users->previousPageUrl())
                                <a href="{{ $users->previousPageUrl() }}" class="px-3.5 py-1.5 border border-gray-250 bg-white hover:bg-gray-50 text-gray-600 rounded-lg text-xs font-semibold transition-colors flex items-center space-x-1">
                                    <i data-lucide="chevron-left" class="w-3.5 h-3.5"></i>
                                    <span>Sebelumnya</span>
                                </a>
                            @endif
                            @if ($users->nextPageUrl())
                                <a href="{{ $users->nextPageUrl() }}" class="px-3.5 py-1.5 border border-gray-250 bg-white hover:bg-gray-50 text-gray-600 rounded-lg text-xs font-semibold transition-colors flex items-center space-x-1">
